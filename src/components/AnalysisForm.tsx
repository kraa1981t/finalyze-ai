...
export default function AnalysisForm({ user, onBegin, onProgress, onResult, onError, lang, settings }: AnalysisFormProps) {
  ...
  const selectAllInCategory = () => {
    const all = SYMBOL_GROUPS[selectedType]?.flatMap(g => g.symbols).filter(s => !hiddenSymbols.includes(s)) || [];
    const areAllSelected = all.every(s => selectedSymbols.includes(s));
