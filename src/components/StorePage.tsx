...
export default function StorePage({ lang, onBack, isDark, onBuyBot }: StorePageProps) {
  ...
  const renderCard = (bot: StoreBot, isFree: boolean) => {
    ...
    return (
      <motion.div
        ...
        className={`${cardBg} ${cardBorder} border rounded-xl p-2.5 flex flex-col gap-1.5 transition-all shadow-lg ${
          isFree
            ? isDark ? 'hover:shadow-emerald-500/20' : 'hover:shadow-emerald-500/10'
            : isDark ? 'hover:shadow-red-500/20' : 'hover:shadow-red-500/10'
        }`}
      >
        {bot.imageData && (
          <div className="relative -mt-2.5 -mx-2.5 mb-0">
            <img src={bot.imageData} alt={bot.name} className="w-full h-24 object-cover rounded-t-xl rounded-b-lg" />
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            <div className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center shadow-md ${isFree ? 'bg-gradient-to-br from-emerald-400 to-emerald-600' : 'bg-gradient-to-br from-red-500 to-red-700'}`}>
              {isFree ? <Tag size={15} className="text-white" /> : <Download size={15} className="text-white" />}
            </div>
            <div className="min-w-0">
              <h3 className={`text-[13px] font-black truncate ${isDark ? 'text-slate-900' : 'text-white'}`}>{bot.name}</h3>
              {bot.fileName && (
          ...
          <span className={`shrink-0 px-2 py-1 rounded-lg text-[10px] font-black uppercase border-2 ${
            isFree
              ...
              : isDark
                ? 'text-red-600 border-red-600/60 bg-red-500/10'
                : 'text-red-400 border-red-400/60 bg-red-500/10'
          }`}>
          ...
          <div>
            ...
            {needToggle && (
              <button
                ...
                className={`mt-0.5 flex items-center gap-0.5 text-[10px] font-black uppercase tracking-wide transition-all ${isFree
                  ? isDark ? 'text-emerald-600 hover:text-emerald-700' : 'text-emerald-400 hover:text-emerald-300'
                  : isDark ? 'text-red-600 hover:text-red-700' : 'text-red-400 hover:text-red-300'}`}
              >
        ...
        {isFree ? (
          ...
          <button
            ...
            className="flex items-center justify-center gap-1.5 w-full py-2 rounded-lg bg-red-500 text-white font-black text-xs uppercase tracking-wider shadow-md shadow-red-500/30 hover:bg-red-600 active:scale-95 transition-all"
          >
  ...
  // Two boxes per level in each column; column scrolls vertically when many
  const renderColumn = (list: StoreBot[], isFree: boolean, headerLabel: string) => (
    <section className="md:max-h-[600px] md:overflow-y-auto custom-scrollbar md:pe-1">
  ...
  // Dashed vertical rosary-style separator between the two columns
  const separator = (
    <div className="hidden md:block shrink-0 self-stretch w-0 mx-2 border-s-2 border-dashed border-slate-500/50" />
  );

  // Free column to the far LEFT, paid to the far RIGHT (physical) — RTL flips the DOM order
  const columns = (() => {
