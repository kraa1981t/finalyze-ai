    ...
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[34px] font-black text-white">{isAr ? 'إدارة الإعلانات' : 'Ads Manager'}</h1>
            <p className="text-[28px] text-slate-400">{isAr ? 'إدارة إعلانات Adsterra لحسابات العملاء' : 'Manage Adsterra ads for client accounts'}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <a
            ...
            className="flex items-center gap-1.5 bg-purple-500/20 hover:bg-purple-500/30 text-purple-400 px-3 py-2 rounded-xl text-[28px] font-bold transition-all"
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 font-bold px-4 py-2.5 rounded-xl text-[28px] transition-all shadow-lg ${
              hasUnsavedChanges
          ...
          <button
            ...
            className="flex items-center gap-2 bg-white/10 hover:bg-white/15 text-white font-bold px-4 py-2.5 rounded-xl text-[28px] transition-all"
          >
            <Plus size={16} />
          ...
          <button
            ...
            className="flex items-center gap-2 bg-primary hover:bg-emerald-500 text-black font-bold px-4 py-2.5 rounded-xl text-[28px] transition-all shadow-lg shadow-emerald-500/20"
          >
            <Plus size={16} />
      ...
      <div className="rounded-3xl border border-indigo-500/20 bg-indigo-500/[0.04] p-5 md:p-6 space-y-4">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[34px] font-black text-white flex items-center gap-2">
                {isAr ? 'موقع إعلانات' : 'Ad Network'}
                <span className="text-lg px-2 py-0.5 rounded-full font-bold bg-indigo-500/20 text-indigo-300">The Moneytizer</span>
              </h2>
              <p className="text-[28px] text-slate-400">{isAr ? 'ربط شبكة إعلانات خارجية (Header Bidding) بموقعك' : 'Link an external ad network (Header Bidding) to your site'}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {mtzLoaded && (
              <span className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[28px] font-black ${
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-[28px] font-black flex items-center gap-1.5 transition-all ${
        ...
        {!mtzLoaded ? (
          <div className="text-center py-6 text-[28px] text-slate-500">{isAr ? 'جاري التحميل...' : 'Loading...'}</div>
        ) : (
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[28px] text-slate-500 font-bold block mb-1">{isAr ? 'معرّف الناشر (Publisher ID)' : 'Publisher ID'}</label>
                <input
                  type="text"
                  ...
                  placeholder="142894"
                  dir="ltr"
                  className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-[28px] text-white placeholder:text-slate-600 focus:outline-none focus:border-indigo-400/50"
                />
              </div>
              <div>
                <label className="text-[28px] text-slate-500 font-bold block mb-1">{isAr ? 'حالة الربط' : 'Link Status'}</label>
                <a
                  href="/ads.txt"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full flex items-center gap-2 bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-[28px] text-indigo-300 hover:border-indigo-400/50 transition-all break-all"
                >
            ...
            <div>
              <label className="text-[28px] text-slate-500 font-bold block mb-1">
              ...
              <textarea
                ...
                rows={4}
                dir="ltr"
                className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-base text-white font-mono placeholder:text-slate-600 focus:outline-none focus:border-indigo-400/50 resize-none"
              />
            </div>
            ...
            <div className="flex items-center gap-2 text-[28px] text-slate-500 bg-white/[0.03] border border-white/5 rounded-lg px-3 py-2">
            ...
            <div className="flex gap-2 pt-1 flex-wrap items-center">
              <button
                ...
                disabled={mtzSaving}
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-[28px] font-black transition-all shadow-lg ${
                  mtzDirty
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-[28px] font-black transition-all bg-red-500/10 text-red-400 hover:bg-red-500/20"
              >
                <Unlink size={13} />
                {isAr ? 'فك الربط وحذف' : 'Unlink & Delete'}
              </button>
              {mtz.enabled && !mtzLinked && (
                <span className="text-[28px] text-amber-400 font-bold">
      ...
      {hasUnsavedChanges && (
        <div className="flex items-center gap-3 bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-3">
          ...
          <p className="text-[28px] text-amber-300 font-bold">
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-white/5 rounded-2xl p-4 border border-white/10 text-center">
          <div className="text-4xl font-black text-white">{ads.length}</div>
          <div className="text-xl text-slate-400 uppercase tracking-wider">{isAr ? 'إجمالي' : 'Total'}</div>
        </div>
        <div className="bg-emerald-500/10 rounded-2xl p-4 border border-emerald-500/20 text-center">
          <div className="text-4xl font-black text-emerald-400">{ads.filter(a => a.enabled && !a.paused).length}</div>
          <div className="text-xl text-emerald-400 uppercase tracking-wider">{isAr ? 'نشط' : 'Active'}</div>
        </div>
        <div className="bg-amber-500/10 rounded-2xl p-4 border border-amber-500/20 text-center">
          <div className="text-4xl font-black text-amber-400">{ads.filter(a => !a.enabled || a.paused).length}</div>
          <div className="text-xl text-amber-400 uppercase tracking-wider">{isAr ? 'معطل' : 'Disabled'}</div>
        </div>
        <div className="bg-blue-500/10 rounded-2xl p-4 border border-blue-500/20 text-center">
          <div className="text-4xl font-black text-blue-400">{ads.filter(a => a.code).length}</div>
          <div className="text-xl text-blue-400 uppercase tracking-wider">{isAr ? 'جاهز' : 'Ready'}</div>
        </div>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
        {[
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-[28px] font-bold whitespace-nowrap transition-all ${
              filterType === tab.key
      ...
      {filteredAds.length === 0 ? (
        <div className="text-center py-16 bg-white/5 rounded-2xl border border-white/10">
          ...
          <p className="text-slate-500 text-xl mt-1">
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="text-[28px] font-black text-white">{ad.name}</span>
                    <span className="text-[34px]">{AD_UNIT_TYPE_ICONS[ad.adUnitType] || '📢'}</span>
                    <span className={`text-lg px-2 py-0.5 rounded-full font-bold uppercase ${networkBadgeClass(getNetworkInfo(ad.type).color)}`}>
                      {getNetworkInfo(ad.type).label}
                    </span>
                    {ad.size && (
                      <span className="text-lg px-2 py-0.5 rounded-full font-bold bg-white/10 text-slate-300">
                        {ad.size}
                      </span>
                    )}
                    {!ad.code && (
                      <span className="text-lg px-2 py-0.5 rounded-full font-bold bg-amber-500/20 text-amber-400">
                        {isAr ? '⚠️ يحتاج كود' : '⚠️ Needs Code'}
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-2 text-xl text-slate-500 flex-wrap">
              ...
              {editAd === ad.id && (
                ...
                >
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[28px] text-slate-500 font-bold block mb-1">{isAr ? 'الاسم' : 'Name'}</label>
                      <input
                        type="text"
                        value={ad.name}
                        onChange={(e) => updateAd(ad.id, { name: e.target.value })}
                        className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-[28px] text-white focus:outline-none focus:border-primary/50"
                      />
                    </div>
                    <div>
                      <label className="text-[28px] text-slate-500 font-bold block mb-1">{isAr ? 'الموضع' : 'Position'}</label>
                      <select
                        value={ad.position}
                    ...
                    <div>
                      <label className="text-[28px] text-slate-500 font-bold block mb-1">{isAr ? 'الحجم' : 'Size'}</label>
                      <input
                        type="text"
                        value={ad.size || ''}
                        onChange={(e) => updateAd(ad.id, { size: e.target.value })}
                        placeholder="728x90, 300x250..."
                        className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-[28px] text-white placeholder:text-slate-600 focus:outline-none focus:border-primary/50"
                      />
                    </div>
                    <div>
                      <label className="text-[28px] text-slate-500 font-bold block mb-1">{isAr ? 'رقم الوحدة' : 'Unit ID'}</label>
                      <input
                        type="text"
                        value={ad.adsterraId || ''}
                  ...
                  <div>
                    <label className="text-[28px] text-slate-500 font-bold block mb-1">{isAr ? 'كود الإعلان' : 'Ad Code'}</label>
                    <textarea
                      value={ad.code}
                      onChange={(e) => {
                      ...
                      rows={4}
                      className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-base text-white font-mono placeholder:text-slate-600 focus:outline-none focus:border-primary/50 resize-none"
                    />
                    {ad.code.trim() && (
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-[34px] font-black text-white">{isAr ? 'قوالب إعلانات سريعة' : 'Quick Ad Templates'}</h3>
                  <p className="text-[28px] text-slate-400 mt-1">{isAr ? 'اختر نوع الإعلان ثم الصق الكود من لوحة الناشر' : 'Choose ad type then paste code from publisher panel'}</p>
                </div>
              ...
              {/* Adsterra presets */}
              <div className="mb-4">
                <p className="text-[28px] font-bold text-purple-400 uppercase tracking-wider mb-2">Adsterra</p>
                <div className="grid grid-cols-2 gap-3">
                  {PRESET_AD_UNITS.filter(p => p.type === 'adsterra').map((preset, idx) => (
                    ...
                    >
                      <span className="text-4xl">{AD_UNIT_TYPE_ICONS[preset.adUnitType || 'banner']}</span>
                      <div className="flex-1">
                        <div className="text-[28px] font-bold text-white">{preset.name}</div>
                        <div className="text-[28px] text-slate-400 mt-0.5">
                          {AD_UNIT_TYPES[preset.adUnitType || 'banner']?.[isAr ? 'ar' : 'en']} • {preset.size}
                        </div>
                        <div className="text-[28px] text-slate-500 mt-1">
              ...
              <div className="mt-4">
                <div className="p-3 bg-purple-500/10 rounded-xl border border-purple-500/20">
                  <p className="text-[28px] text-purple-300 font-bold mb-1">Adsterra — {isAr ? 'كيف تحصل على الأكواد:' : 'How to get codes:'}</p>
                  <ol className="text-[28px] text-purple-400/70 space-y-1 list-decimal list-inside">
      ...
      <AnimatePresence>
        {showAdd && (
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-[34px] font-black text-white">{isAr ? 'إضافة إعلان مخصص' : 'Add Custom Ad'}</h3>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[28px] text-slate-400 font-bold block mb-1.5">{isAr ? 'اسم الإعلان' : 'Ad Name'}</label>
                  <input
                    type="text"
                    value={newAd.name}
                    onChange={(e) => setNewAd(prev => ({ ...prev, name: e.target.value }))}
                    placeholder={isAr ? 'مثال: بانر الرئيسية' : 'e.g. Homepage Banner'}
                    className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-[28px] text-white placeholder:text-slate-500 focus:outline-none focus:border-primary/50"
                  />
                </div>

                <div>
                  <label className="text-[28px] text-slate-400 font-bold block mb-1.5">{isAr ? 'الشبكة الإعلانية' : 'Ad Network'}</label>
                  <div className="flex gap-2 flex-wrap">
                    {[
                      ...
                      <button
                        ...
                        className={`flex-1 min-w-[100px] py-2.5 rounded-xl text-[28px] font-bold transition-all border ${
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[28px] text-slate-400 font-bold block mb-1.5">{isAr ? 'النوع' : 'Type'}</label>
                    <select
                      value={newAd.adUnitType}
                      onChange={(e) => setNewAd(prev => ({ ...prev, adUnitType: e.target.value as Ad['adUnitType'] }))}
                      className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-[28px] text-white focus:outline-none focus:border-primary/50"
                    >
                  ...
                  <div>
                    <label className="text-[28px] text-slate-400 font-bold block mb-1.5">{isAr ? 'الموضع' : 'Position'}</label>
                    <select
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[28px] text-slate-400 font-bold block mb-1.5">{isAr ? 'الحجم' : 'Size'}</label>
                    <input
                      type="text"
                      value={newAd.size}
                  ...
                  <div>
                    <label className="text-[28px] text-slate-400 font-bold block mb-1.5">{isAr ? 'رقم الوحدة (اختياري)' : 'Unit ID (optional)'}</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="text-[28px] text-slate-400 font-bold block mb-1.5">{isAr ? 'كود الإعلان' : 'Ad Code'}</label>
                  <textarea
                    value={newAd.code}
                    ...
                    rows={5}
                    className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-base text-white font-mono placeholder:text-slate-500 focus:outline-none focus:border-primary/50 resize-none"
                  />
                  {newAd.code.trim() && (
                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                      <span className={`text-lg px-2 py-1 rounded-full font-bold uppercase ${networkBadgeClass(getNetworkInfo(newAd.type).color)}`}>
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    ...
                    className="flex-1 py-3 bg-white/5 border border-white/10 text-slate-400 hover:bg-white/10 font-bold rounded-xl text-[28px] transition-all"
                  >
                    {isAr ? 'إلغاء' : 'Cancel'}
                  </button>
                  <button
                    ...
                    className="flex-1 py-3 bg-primary hover:bg-emerald-500 text-black font-bold rounded-xl text-[28px] transition-all shadow-lg shadow-emerald-500/20 disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    {isAr ? 'إضافة' : 'Add'}
                  </button>
                </div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-[34px] font-black text-white">{isAr ? 'تعيين إعلان لعملاء' : 'Assign Ad to Clients'}</h3>
                  <p className="text-[28px] text-slate-400 mt-1">{assignModal.name}</p>
                </div>
              ...
              {clientEmails.length === 0 ? (
                <div className="text-center py-8 bg-white/5 rounded-2xl">
                  ...
                  <p className="text-slate-400 text-[28px] font-bold">{isAr ? 'لا يوجد عملاء مسجلين' : 'No registered clients'}</p>
                  <p className="text-slate-500 text-xl mt-1">{isAr ? 'سيظهر القائمة عندما يسجل عملاء جدد' : 'List will appear when new clients register'}</p>
                </div>
              ) : (
                <div className="space-y-2">
                  ...
                  >
                    <span className="text-[28px] font-bold">{isAr ? 'تحديد الكل' : 'Select All'}</span>
                  ...
                  {clientEmails.map(email => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center text-white text-xl font-bold">
                            {email.charAt(0).toUpperCase()}
                          </div>
                          <span className="text-[28px] truncate">{email}</span>
                        </div>
              ...
              <button
                ...
                className="w-full mt-4 py-3 bg-white/5 border border-white/10 text-slate-400 hover:bg-white/10 font-bold rounded-xl text-[28px] transition-all"
              >
                {isAr ? 'إغلاق' : 'Close'}
              </button>
            </motion.div>
        ...
        {codeModal && (
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-[34px] font-black text-white">{isAr ? 'كود الإعلان' : 'Ad Code'}</h3>
                  <p className="text-[28px] text-slate-400 mt-1">{codeModal.name}</p>
                </div>
              ...
              {codeModal.code ? (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[28px] text-slate-500 uppercase tracking-wider">{isAr ? 'كود الإعلان' : 'Ad Code'}</span>
                    <button
                      onClick={() => copyCode(codeModal.code)}
                      className="flex items-center gap-1 text-[28px] text-primary hover:text-emerald-400 transition-colors font-bold"
                    >
                      {copied ? <Check size={14} /> : <Copy size={14} />}
                      {copied ? (isAr ? 'تم النسخ' : 'Copied') : (isAr ? 'نسخ الكود' : 'Copy Code')}
                    </button>
                  </div>
                  <pre className="bg-black/40 rounded-xl p-4 text-base text-slate-300 overflow-x-auto max-h-48 font-mono whitespace-pre-wrap break-all">{codeModal.code}</pre>
                </div>
              ) : (
                <div className="text-center py-8 bg-white/5 rounded-2xl">
                  <Code size={32} className="mx-auto text-amber-400 mb-3" />
                  <p className="text-amber-300 text-[28px] font-bold">{isAr ? 'لم يتم إضافة الكود بعد' : 'Code not added yet'}</p>
                  <p className="text-slate-500 text-xl mt-1">{isAr ? 'اضغطتعديل لإضافة كود الإعلان' : 'Click edit to add the ad code'}</p>
                </div>
              )}
            </motion.div>
          </motion.div>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
            className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-[200] flex items-center gap-3 px-6 py-3 rounded-2xl font-bold text-[28px] shadow-2xl backdrop-blur-xl border ${
              toast.type === 'ok'
