  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="w-full max-w-4xl mx-auto">
        <div className="h-[300px] md:h-[400px] bg-slate-950 rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/5 relative">
          <TradingViewWidget symbol={selectedResult.symbol} />
          <div className="absolute top-8 left-8 z-10 flex items-center gap-3">
             <div className="px-5 py-2.5 bg-black/80 backdrop-blur-2xl rounded-full border border-white/20 text-white text-[11px] font-black uppercase tracking-[0.2em] flex items-center gap-3 shadow-2xl">
               <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_15px_rgba(16,185,129,1)]" />
               <span className="text-white">LIVE MARKET: {selectedResult.symbol}</span>
             </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className={cn("flex items-center justify-between px-4", isRTL ? "flex-row" : "flex-row-reverse")}>
          <span className="text-xs font-bold text-slate-300 font-mono">Analyzed: {results.length}</span>
                ...
                <div 
                  className={cn(
                    ...
                    isSelected 
                      ...
                      : "border-white/5 bg-brand-alt hover:border-white/10"
                  )}
                  onClick={() => setSelectedIndex(idx)}
                >
                  <div className={cn("flex flex-col lg:flex-row items-stretch min-h-[140px]", isRTL ? "lg:flex-row" : "lg:flex-row-reverse")}>
                    {/* Symbol & Intensity */}
                    <div className="w-full lg:w-[280px] p-8 bg-black/40 border-b lg:border-b-0 border-white/10 flex flex-col justify-center gap-3">
                      <div className="flex items-center gap-4">
                        <div className={cn(
                          "w-12 h-12 rounded-2xl flex items-center justify-center transition-all shadow-xl",
                          isSelected ? "bg-primary text-white scale-110 rotate-3" : "bg-white/10 text-slate-300"
                        )}>
                          <span className="text-lg font-black">{idx + 1}</span>
                        </div>
                        <span className="text-3xl font-black text-white tracking-tighter font-display italic">{res.symbol}</span>
                      </div>
                    ...
                    <div className="w-full lg:w-[320px] p-8 flex flex-col justify-center items-center lg:items-start gap-2 border-b lg:border-b-0 border-white/10">
                      <div className="text-[11px] font-black uppercase tracking-[0.2em] text-white">{t.finalDecision}</div>
                    ...
                    <div className="flex-1 p-8 flex flex-col justify-center relative">
                      <div className="text-[11px] font-black uppercase tracking-[0.2em] text-white mb-2">{t.reasons}</div>
                      ...
                      <div className="mt-6 flex items-center gap-8 opacity-80 group-hover:opacity-100 transition-opacity">
                         <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-black text-white">{res.technicalScore}%</span>
                         </div>
                         <div className="flex items-center gap-2">
                            <MessageSquare size={14} className="text-secondary" />
                            <span className="text-[10px] font-black text-white">{res.sentimentScore}%</span>
                         </div>
                         {res.historicalMatch && (
                           <div className="flex items-center gap-2">
        ...
        {showDetail && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="relative w-full max-w-4xl bg-brand-bg border border-white/10 rounded-[3rem] overflow-hidden shadow-2xl shadow-black/50 overflow-y-auto max-h-[90vh]"
            >
              <div className="p-8 md:p-12 space-y-10">
                {/* Header */}
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-6">
                    <div className="w-16 h-16 bg-white/10 rounded-3xl flex items-center justify-center text-primary border border-white/20">
                      {selectedResult.symbol.includes('USD') ? <Globe size={32} /> : <Zap size={32} />}
                    </div>
                    <div>
                      <h2 className="text-4xl font-black tracking-tight text-white italic mb-1">{selectedResult.symbol}</h2>
                      <div className="flex items-center gap-4">
                         ...
                         <span className="text-slate-300 font-mono text-[10px]">{new Date(selectedResult.timestamp).toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                  <button 
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                   ...
                   <div className="lg:col-span-2 space-y-8">
                      <div className="p-8 bg-brand-alt border border-white/5 rounded-[2rem] space-y-4">
                   ...
                   {/* Right: Scores & Stats */}
                   <div className="space-y-6">
                      <div className="p-8 bg-brand-alt border border-white/5 rounded-[2rem] flex flex-col items-center gap-6">
                         <div className="text-center">
                            <div className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-4">{t.confidence}</div>
